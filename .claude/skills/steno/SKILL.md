...
Features:
  - shadcn-inspired theme (light/dark mode)
...
The generated HTML uses the shadcn theme from `assets/steno-transcript.css` and includes:
...
Steno uses a shadcn-inspired theme (`assets/steno-transcript.css`) for HTML exports. The theme provides:

- **Light/dark mode**: Automatic detection + manual toggle
- **OKLch colors**: Modern color space for consistency
- **Message styling**: Distinct colors for user, assistant, thinking, and tool messages
- **Code highlighting**: Clean monospace styling for code blocks
- **Smooth animations**: Expand/collapse and hover effects
- **Print-friendly**: Clean output when printing

**Apply theme to claude-code-transcripts output:**

The standalone CSS can also style transcripts generated by `claude-code-transcripts`. After generating:

```
> steno:transcript +generate

Generating transcripts...
Done: .steno/transcripts/

> steno:transcript +theme

Applying shadcn theme to transcripts...
  Injected: steno-transcript.css
...
<head>
  <!-- Add before </head> -->
  <link rel="stylesheet" href="path/to/steno-transcript.css">
</head>
<body class="dark"> <!-- or omit for light mode -->
```

Or copy the CSS directly into a `<style>` tag.
...
```css
:root {
  --primary: oklch(0.6 0.2 260);  /* Custom blue */
